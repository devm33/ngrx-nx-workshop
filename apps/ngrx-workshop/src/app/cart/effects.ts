...
import { switchMap, catchError, map, mergeMap, tap, withLatestFrom, exhaustMap } from 'rxjs/operators';
...
export class CartEffects {
    ...
    this.actions$.pipe(
      ...
      exhaustMap(([, cartItems]) =>
        this.cartService.removeAll().pipe(
          ...
          catchError(() =>
            of(actions.removeAllFromCartError({ cartItems: cartItems || [] }))
          )
        )
      )
    )
  ...
  purchaseCartProducts$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(({ cartProducts }) =>
        cartProducts.map((p): [string, number] => [p.id, p.quantity])
      ),
      exhaustMap(cartItems =>
